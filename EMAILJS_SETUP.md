...
1. **Шаблон для владельца** — письмо с данными клиента приходит вам
2. **Шаблон для пользователя** — автоматический ответ клиенту на его email

---

## Шаг 0: Настройка переменных окружения (.env.local)

**Все ID берутся из аккаунта клиента в EmailJS Dashboard.** Убедитесь, что используете данные именно того аккаунта, которому принадлежит сайт.

1. Скопируйте `.env.example` в `.env.local` в корне проекта
2. Войдите в EmailJS Dashboard клиента: https://dashboard.emailjs.com/
3. Заполните переменные:

| Переменная | Где найти |
|------------|-----------|
| `NEXT_PUBLIC_EMAILJS_SERVICE_ID` | **Email Services** → выберите сервис → скопируйте Service ID (начинается с `service_`) |
| `NEXT_PUBLIC_EMAILJS_TEMPLATE_OWNER` | **Email Templates** → шаблон для владельца → Template ID |
| `NEXT_PUBLIC_EMAILJS_TEMPLATE_REPLY` | **Email Templates** → шаблон автоответа → Template ID |
| `NEXT_PUBLIC_EMAILJS_PUBLIC_KEY` | **Account** → API Keys → Public Key |

4. Перезапустите dev-сервер (`npm run dev`)

**Важно:** Service ID, Template ID и Public Key должны принадлежать одному аккаунту EmailJS. Ошибка «The service ID not found» означает, что Service ID от другого аккаунта.
...
2. Найдите шаблон или создайте новый
3. Скопируйте Template ID в `NEXT_PUBLIC_EMAILJS_TEMPLATE_OWNER` в `.env.local`
...
5. Скопируйте Template ID в `NEXT_PUBLIC_EMAILJS_TEMPLATE_REPLY` в `.env.local`
...
3. Убедитесь, что шаблон автоответа активен и правильно настроен

### Проверка настроек шаблона:

1. Откройте шаблон автоответа в EmailJS Dashboard
...
3. Скопируйте Service ID в `NEXT_PUBLIC_EMAILJS_SERVICE_ID` в `.env.local`
...
✅ **Код использует переменные окружения.** Все ID настраиваются через `.env.local`:
```
NEXT_PUBLIC_EMAILJS_SERVICE_ID=service_xxxxx
NEXT_PUBLIC_EMAILJS_TEMPLATE_OWNER=template_xxxxx
NEXT_PUBLIC_EMAILJS_TEMPLATE_REPLY=template_xxxxx
...
1. **Service ID:** из `.env.local` — Email Services в Dashboard
2. **Template ID владельца:** из `.env.local` — Email Templates
3. **Template ID автоответа:** из `.env.local` — Email Templates
4. **Public Key:** из `.env.local` — Account → API Keys
...
1. **Пользователь заполняет форму** на сайте (имя, возраст, телефон, email, комментарий)
...
5. Проверьте, что в шаблоне для пользователя поле "To Email" содержит `{{email}}` (отправляется из кода)
...
A: Проверьте, что:
- Template ID для пользователя правильный в `.env.local`
- В шаблоне поле "To Email" содержит `{{email}}` (БЕЗ кавычек!)
...
A: Это означает, что шаблон автоответа или Service ID настроен неправильно. Проверьте:

1. **Ошибка "Service ID not found":** Service ID в `.env.local` должен быть из того же аккаунта EmailJS, что и Public Key. Найти: Dashboard → Email Services

2. **Откройте шаблон автоответа в EmailJS Dashboard**
3. **Проверьте поле "To Email":**
   - Должно быть: `{{email}}` (без кавычек, без пробелов)
   - НЕ должно быть: `"{{email}}"` или `{{ email }}` или ваш email
   
4. **Проверьте переменные в шаблоне:**
   - В HTML используйте: `{{user_name}}` и `{{email}}`
   - Переменные должны совпадать с теми, что отправляются из кода
   
5. **Проверьте, что шаблон активен:**
